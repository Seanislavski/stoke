...
import { resolveReport, dismissReport, reopenReport } from '@/app/actions/reports'
...
export default function ReportActions({
  ...
  if (status !== 'open') {
    return (
      <button
        disabled={pending}
        onClick={() => startTransition(() => void reopenReport(reportId, communitySlug))}
        className="text-xs px-2.5 py-1 bg-stone-100 text-stone-600 hover:bg-stone-200 disabled:opacity-50 rounded-lg font-medium transition-colors shrink-0"
      >
        Re-open
      </button>
    )
