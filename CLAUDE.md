...
- **⚠️ NOT yet exercised end-to-end (as of 07/28).** Probe of `discord_captures`: only 4 rows, all Sean's own — 3 dismissed, and the one filed as a question is **claimed**, so promoting it takes the OLD in-app path. **Nothing in the DB currently triggers the DM branch.** To test: capture a post → grant credit → **do NOT claim** → file as question → Publish as QotW → DM within 60s. If it doesn't land, the queue row's `attempts`/`last_error` is the diagnostic.
