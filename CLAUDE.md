...
- **⏳ STILL UNTESTED: the routing branch only** (`if (capture && !capture.claimed_by)` in `publishExistingQuestion`). Probe of `discord_captures` showed 4 rows, all Sean's own — 3 dismissed, and the one filed as a question is **claimed**, so promoting it takes the OLD in-app path. Nothing in the DB triggers the DM branch; it gets exercised on the first real unclaimed promotion.
