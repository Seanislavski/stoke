...
- **⚠️ curl trap on this Windows box: `-o /dev/null` ALONE yields `status=000` / exit 43** — it is not `-I` that breaks it, as an earlier note here claimed. To read status/headers use **`curl -s -D - <url> | head`**; to read a body just `curl -s <url>`. (Fired twice on 08/05/2026.)
