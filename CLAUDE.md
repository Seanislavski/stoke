...
- **⚠️ NEVER `void` A SUPABASE QUERY BUILDER — IT NEVER RUNS.** The request is only sent inside `then()` (postgrest-js), so `void admin.from('x').insert(...)` silently does nothing. `await` it, or put it inside a `void (async () => { await ... })()`. Found 09/21/2026: the contest-winner bell insert had never fired (fixed `be0f5db`).
- **⚠️⚠️ ADDING A COLUMN TO A `select()` THAT FEEDS `.single()` + `notFound()` MAKES A MISSING COLUMN A SITE-WIDE 404 (09/01/2026).** The community page does `.select(...).eq('slug', slug).single()` then `if (!community) notFound()`. Ship code selecting a not-yet-created column and PostgREST returns `42703`, `community` is null, and **every community page 404s** — the failure is on the main content route, not on the new feature. **So expand-then-contract applies to READS, not just writes: the migration must be applied BEFORE the code that selects the column is pushed.** Hold the push, run the migration, probe for the column, then push. (Done this way for `onboarding_dismissed_at`.)
...
- **Read-only DB probe without a script:** `curl -H "apikey: $KEY" -H "Authorization: Bearer $KEY" "<url>/rest/v1/<table>?select=<col>&limit=1"` with the service-role key from `apps/web/.env.local`. Sidesteps the throwaway-`.mjs` ESM-resolution trap entirely and leaves nothing to clean up. Missing column → `42703`; missing table → `PGRST205`. Running the probe **before** a migration gives you the before-picture, so the same two queries afterwards are the verification.
- **⚠️ curl trap on this Windows box: `-o <ANY path>` yields `status=000` / exit 43** — not just `-o /dev/null`, and not `-I`, as two earlier versions of this note claimed. `-o /tmp/x` fails identically (08/06/2026). To read status/headers use **`curl -s -D - <url> | head`**; to read a body just `curl -s <url>`.
- **⚠️ RE-READ live state immediately before mutating it on Sean's behalf — he may have acted between the recommendation and the execution.** (08/07: recommended flipping the contest to `draft`, he clicked it himself in the UI, and my service-role PATCH four minutes later was a silent no-op.) The tell that it was *him* and not a DB trigger: a service-role write has **no `auth.uid()`**, so a trigger could never have stamped `actor_id` with his profile id — the `audit_log` row had to come from the app action. **Corollary: mutating via direct PostgREST skips the `logAction` call the UI action would have made**, so prefer telling him where to click; if you do write directly, check whether an audit row is owed before adding one (here it wasn't — his click already logged it).
- **⚠️ A GREEN "Success. No rows returned" IS NOT EVIDENCE THE RIGHT SQL RAN.** Migrations here are written `add column if not exists`, so **re-running an older migration reports success and does nothing** — indistinguishable from a new one succeeding. Cost three attempts on 08/06 because the Supabase SQL editor tab still held the *previous* migration. **Always verify by probing for the NEW object**, and when handing Sean SQL, name the tell (a column/function only the new script creates).
- **⚠️ The `42P01` wrong-project safety net DISAPPEARS for migrations that only touch tables common to several projects.** The 08/05 contests migration failed loudly in Plish because `communities` is Stoke-only; a `profiles`-only migration would have **succeeded silently there**, since Plish has `profiles` too — and plpgsql function bodies aren't validated for column references at creation time, so even a trigger rewrite reports success. Probe after every migration, not just the ones you expect to fail.
- **⚠️ The Supabase SQL editor flags ANY script containing `DROP` as "destructive operations" — it pattern-matches the keyword and cannot tell `drop index` from `drop table`.** The warning is unavoidable when you legitimately need to replace an index, so don't reword the script hoping it clears. Answer it instead: say what is actually at risk (an index is a lookup structure, not data), wrap the script in `begin;`/`commit;` so a mid-script failure can't leave uniqueness unenforced, and hand over concrete rollback SQL. ⚠️ Note when a rollback **expires** — restoring a *stricter* index only works while no row violates it yet. (08/08/2026, the testimonial-capture migration.)
- **⚠️ CLONING A PIPELINE ONTO A NEW TABLE MEANS RE-READING THE ORIGINAL'S GOTCHA LIST AND TICKING EACH ITEM OFF — check the FK cascade FIRST.** Building the testimonial capture funnel into `reviews` reproduced a trap fixed on the Q&A side back in July: `discord_captures.review_id` is `on delete set null`, so deleting filed content resets the pointer and the capture **bounces back into the queue as if never filed**. The Q&A path solves it by stamping `dismissed_at` *before* deleting (`deleteQuestion`); `deleteReview` didn't, until it did. A parallel funnel inherits the parent's bugs, not its fixes.
- **⚠️ NEVER use `ilike`/`LIKE` for an equality check on user-supplied text** — `_` and `%` are wildcards. `profiles.username` permits underscores (`Avi_Rose` is a real member), so `ilike('avi_rose')` also matches `AviXRose` and wrongly reports it taken. Use a generated lowercase column + `.eq()` (see `profiles.username_lower`).
- **Supabase auth probes that need no dashboard access:** `GET /auth/v1/settings` with the **anon** key returns the full enabled-provider map (`{"external":{"discord":true,…}}`) — the way to verify an OAuth provider is on. And `GET /auth/v1/authorize?provider=<p>&redirect_to=<url>` 302s with the **complete provider URL in the `Location` header**, so you can read the exact `client_id`, `redirect_uri` and `scope` being sent. That turns "is it configured right?" from a conversation into a measurement.
- **Supabase auto-links an OAuth identity into an existing account when the email matches and is confirmed** (verified 08/06: signing in with Discord on an existing account's email produced no duplicate profile). "Allow manual linking" is a *separate* setting governing the `linkIdentity()` API for deliberate linking — it does not control automatic linking.
- `favicon.ico` in `app/` always overrides `icon.svg` — delete `favicon.ico` when adding SVG favicon

## Organizer Onboarding Checklist (09/01/2026 — `383110a`, LIVE)
- `components/community/OnboardingChecklist.tsx`, rendered on `communities/[slug]/page.tsx` for `isMod` only. Four steps (welcome post · channel · first member · event); auto-hides only when **all four** are done.
- **Dismissal lives in `communities.onboarding_dismissed_at` (timestamptz, nullable; null = never dismissed)**, set by `dismissOnboarding(communityId, slug)` in `app/actions/community.ts` behind the `getCallerRole` chokepoint. Migration `20260901000000_onboarding_dismissed.sql`. The page guard is `{isMod && !community.onboarding_dismissed_at && (`.
- **⚠️ IT USED TO BE `localStorage` AND THAT IS WHY IT KEPT COMING BACK.** Sean: *"I have clicked the x many times but it reappears regularly"* — a per-browser, per-device key (`stoke_onboarding_dismissed_{slug}`) means a dismissal in Firefox is invisible in Chrome, on a phone, or after cleared site data. **★ A PREFERENCE THE USER EXPRESSES BY CLICKING IS A FACT ABOUT THE ACCOUNT/OBJECT, NOT ABOUT THE BROWSER** — if it is worth remembering it is worth a column.
- The dismiss control is a **labelled "Hide" button**, not a bare grey ✕. Body Doubling sat at 3 of 4 steps (1 post, 2 channels, 92 members, **0 events**), so it was never going to auto-hide — dismissal was the only exit and it had no memory.
- No `logAction`: hiding your own dashboard hint is not a moderation act and would be noise in the community audit log.
...
- **⚠️ `npm run build` FAILS LOCALLY at the postbuild `cp` step — the COMPILE still succeeded.** Error: `cp: cannot create directory '.next/standalone/.next/static': No such file or directory`, because in this monorepo Next writes standalone output to `.next/standalone/apps/web/`, not `.next/standalone/`. Railway builds fine with the same script. **Read WHICH STEP failed before calling a build broken** — look for `✓ Compiled successfully` and the route table above the npm error. (08/05/2026.)
...
- **`docs/reference/discord-identity.md`** — Discord handles on profiles, sign in with Discord, connect/disconnect an existing account, and automatic credit for captured posts; the rewritten `handle_new_user` + `derive_username`; `profiles.discord_user_id/discord_username/show_discord/username_chosen`, `communities.show_discord_handles`. ⚠️ **Discord sends the handle as `full_name`, NOT `preferred_username`** (`name` is the legacy `user#0` form, `custom_claims.global_name` is the display name) — reading the wrong key nulled every handle. ⚠️ Connecting is deliberately separate from showing; disconnecting never un-claims posts; unlink refuses when Discord is the only sign-in method. ⚠️ `Invalid OAuth2 redirect_uri` is always Discord-side — delete and re-paste, never inspect.
- **`docs/reference/ops-and-launch.md`** — hosted static assets, launch history + funnel diagnosis, seed/cleanup scripts, **the off-platform library export + weekly Proton Drive backup** (`scripts/export-library.mjs`, `scripts/backup-library.cmd`, scheduled task `Stoke library backup`). ⚠️ The export's attribution must let `kb_answers.attribution` win over the profile lookup, or unclaimed Discord captures get credited to the bot. ⚠️ **The middleware `config.matcher` must exclude EVERY hosted static file extension** (`pdf|html|ico|txt|xml|webmanifest`) — a new public file type that isn't listed gets auth-gated and redirects logged-out visitors to `/login`. Always test hosted assets **while logged out**, checking content markers (`%PDF-`) not just status codes.
...
- **⚠️ EVERY BADGE COUNT MUST MIRROR THE QUERY ON THE PAGE IT LINKS TO.** A badge and its destination are ONE feature implemented as TWO queries in TWO files, and they drift silently — only a human clicking notices. 08/08/2026: filtering the review queue to `kind='qa'` without filtering the count in `communities/[slug]/page.tsx` made a captured testimonial show as "1" next to **Review queue** and sent Sean to an empty list. Whenever you narrow a queue's query, find every count that points at it. Same family as the audit-log dual-surface rule. `CommunityGear` now takes `reviewQueueCount` + `testimonialCount` separately, with the outer badge as the sum.
- **⚠️ THE GEAR DOES NOT EXIST ON EVERY PAGE — check the route's layout before telling anyone where to click.** `AppNav` (avatar + gear as ONE button, `NotificationsBell` immediately left of it) renders only on `(app)` routes — `/home`, `/communities`, community pages. The standalone marketing-ish routes — **`/guide`, `/guide/capture`, `/guide/discord`, `/about`, `/changelog`** — each render their own slim header (wordmark + plain text links), with **no bell, no avatar, no gear**. Deliberate, not a bug, but it means a gear-menu entry pointing *to* a guide is unreachable *from* that guide, and step-by-step instructions that say "click your avatar" are wrong on those pages. (Cost a confused round-trip 08/02/2026.)
...
## Usernames (policy set 08/06/2026 — Sean chose "lowercase only")
- **New usernames are LOWERCASE.** `derive_username()` lowercases, both forms (`signup`, `ChooseUsernameForm`) lowercase **as you type** rather than silently on save, and their `pattern` is `[a-z0-9_]+`. Sean chose this over my recommendation (preserve typed case) — implement what he chose, don't re-litigate.
- **43 of 82 existing members legitimately have capitals** (`Sinaratheus`, `SammieNutkins`). They keep them. The asymmetry is permanent and fine — **do NOT migrate old usernames down**, it would change 43 people's profile URLs and their identity as they know it.
- **`profiles.username_lower`** — generated `lower(username)`, **unique index** `profiles_username_lower_key`. It is the query key for every case-insensitive username lookup, and the unique index makes two accounts differing only by case impossible **in the database** rather than by app-code vigilance.
- `/profile/{username}` looks up by `username_lower`, then **redirects to the stored spelling** so a profile has one canonical address. Only two username lookups exist in the codebase — that page and `chooseUsername()`; keep it that way.
- `chooseUsername()` refuses once `username_chosen` is true, so it can never become a general rename. Usernames are permanent, which is what the profile page promises everyone.
...
## ⚠️ THE SHAREABLE-LINK RULE (generalized 08/05/2026 after it bit a THIRD time)
- **Any new members-only page that could ever be linked from outside needs its join-gate decision made AT BUILD TIME.** Not after someone posts the link.
- The older rule below — *"whenever you gate a page, check it against what the logged-out preview already exposes"* — is necessary but **does not fire for a brand-new feature**, because a feature with no preview route has nothing to compare against. That's exactly how contests shipped (08/05) with `if (!isMember && !isMod) notFound()` and no preview: logged-out visitors **307 → `/login`**, signed-in non-members get a bare 404. Same funnel bug as Q&A in July, third occurrence.
- **The test:** *"Sean posts this link to 13,000 Discord members — what do they see?"* If the answer is a login wall or a 404, the page needs a gate showing enough to justify joining (title/brief/context + Join button), with the member-only substance still withheld.
- Reference implementation: `components/knowledge/QuestionJoinGate.tsx`. ⚠️ It calls `router.refresh()` after an open join because `joinCommunity` doesn't revalidate the gated page.
- **`datetime-local` inputs send a NAIVE string with no zone, and Postgres `timestamptz` reads it as UTC.** An organizer in ET typing `2026-09-01 00:00` gets a deadline of Aug 31, 8:00 PM their time. Convert to a real ISO string with offset client-side before submitting. Worst where the timestamp is *enforced* rather than displayed (e.g. `contests.submissions_close_at`).
- **⚠️ SECOND DIMENSION, ADDED 08/06 AFTER IT BIT A FOURTH TIME — the URL must also survive being RETYPED, PASTED WRONG, or AUTO-CAPITALIZED.** The rule above asks *who can see* a page; this asks whether the address *resolves at all*. Sean himself hit `/communities/**B**odydoublingcom/…` → 404, because slug lookups were `.eq()` and case-sensitive. Mobile keyboards auto-capitalize; people retype links from screenshots.
  - **The two halves needed OPPOSITE fixes, and treating them alike would have broken things.** Community **slugs** are stored lowercase → middleware 308s the URL **down** to lowercase (`middleware.ts`, top of the function, before any auth work; scoped to `/communities/<slug>` and `/preview/<slug>`). Profile **usernames** are legitimately mixed-case (43 of 82 members) → lowercasing the URL would break 43 working links, so it needs a lowercase **lookup key** (`profiles.username_lower`, generated + unique) plus a redirect **up** to the stored spelling.
  - **⚠️ The middleware redirect must NOT touch `/profile/<username>`** — the comment in `middleware.ts` says why. Lowercasing there breaks every mixed-case profile link on the platform.
  - **A wrong URL also has to LAND somewhere survivable.** The app had **no `not-found.tsx` at all** until 08/06, so every bad link hit Next's unstyled default — no branding, no way back, near-invisible on the stone background. `app/not-found.tsx` now varies its links by auth state, because `/home`, `/communities` and `/support` all bounce a logged-out visitor into a login wall (a second dead end).
...
- **⚠️ THE LOGGED-OUT GATE IS NOW THE SAME RULE (07/28, `ba162a1`)** — `preview/[slug]/questions/[questionId]/page.tsx` was narrower (QotW or `is_public` only), so a logged-out visitor could reach only QotW questions while a signed-in stranger could read them all. Both are now `published AND (is_listed || is_public || isQotw)`. **Keep them identical; edit them together.** This is what makes Silas's `/library` links land on content instead of `/login` (see `docs/reference/discord-capture-pipeline.md`).
- **⏳ Join mode is still part of `/library`'s design, not a separate setting** — `join_mode='open'` (current) is what makes a `/library` link end in a read rather than an approval wall. Results now narrow automatically if the community is unlisted, but going closed still changes what happens *after* the click.
...
- **⚠️ Hash links do NOT scroll on INITIAL page load in the App Router.** The browser attempts its jump before the streamed server content exists, finds nothing, and gives up — so `/settings#audit-log` lands at the top of the page. **In-page clicks work fine** (the target is already rendered), which is why this hides for months: the gear menu's audit-log deep link had been going nowhere since it shipped. Fix: `components/ScrollToHash.tsx` (client, renders null, retries `getElementById` every 50ms for ~2s). Mount it on any page with anchor targets.
- **⚠️ `scroll-mt-*` must clear EVERY stacked sticky bar, not just the header.** Community settings has two: `AppNav` (`sticky top-0`, `h-14` = 56px) and the section quick-nav (`sticky top-14`, ~40px) ≈ 96px of chrome. Sections were `scroll-mt-20` (80px), so jumping to one tucked its heading **under** the nav. Now `scroll-mt-28` (112px). If either bar changes height, the offset must move with it — a comment in `settings/page.tsx` ties them together.
- **⚠️ A string absent from the served HTML is NOT proof it didn't ship.** `/login` and `/signup` are statically generated and use `useSearchParams()` inside `<Suspense>`, which bails out of prerendering — so client-only subtrees appear in the **JS chunk**, not the HTML. To verify: pull the chunk URLs out of the HTML and grep those. (Used 08/06 to confirm the Discord button deployed.)
- **`components/LocalDate.tsx` renders in the VIEWER's timezone** (`new Date(ts).toLocaleDateString()`), so every timestamp is already correct for a global membership — a member in Tokyo sees JST. **Do NOT "fix" it into a fixed zone.** It prints no zone label (an opt-in `withZone` prop is proposed, unbuilt); 8 usages / 6 files. The unsolved half is the **organizer's INPUT side** — `datetime-local` converts through the *browser's* zone, so the same typed time means different things depending on where the laptop is standing. Sean's reference zone for anything he sets is **Eastern Time** ("where I am from, and I started the community") — arbitrary-but-stable beats correct-but-drifting. **⚠️ Live 08/10–11/03/2026 he is in Denmark (Aalborg), ahead of ET by 6h — but 5h for one week, since Denmark falls back Oct 25 and the US Nov 1.**
- **⚠️ A FILTER'S EMPTY STATE IS PART OF THE FILTER — design both at once.** Three bugs in one feature on 08/08/2026 were all the same mistake (a screen saying "empty" when it wasn't): a badge counting rows its destination filtered out, a delete that bounced an item back into a queue, and `ReviewsManager` opening on a hardcoded **Pending** tab so a community with reviews but nothing pending read as "No reviews here" with its content one tab away. All three passed `tsc`; all three were found by Sean clicking. It now defaults to Pending only when something *is* pending. **A default filter is a claim about what the user came to do — a dedicated page and a settings queue are different visits.**
- **⚠️ A DEFENSIVE NO-OP NEEDS ITS OWN UI STRING, or the interface lies on exactly the path you were careful about.** `reattributeReview` declines when a claimer already has a review in that scope — correct, and silent. Without returning `credited: false`, the claim page would have reported a credit it never delivered. Any guard that quietly *doesn't* act needs a matching message saying so.
- **⚠️ A gated route's contents cannot be deploy-verified from outside** (middleware 307s to `/login` before the page runs, so shipped and nonexistent look identical). Verify via a **public** marker in the same build — e.g. a `/changelog` entry, or a server-rendered attribute on `/signup` — and hand the gated check to Sean. Where a public equivalent of the gated route exists (`/preview/<slug>` for `/communities/<slug>`), test that instead.
...
- **⚠️ `.claude/name`, `.claude/settings.json`, `images/` and the QotW PDF are TRACKED as of `8e826f3` (07/29).** The `docs/claude-md-archive/` files record a convention of "deliberately left untracked" at 07/13, 07/15 and 07/28 — **that is now reversed; do not re-untrack them.** Only `.claude/settings.local.json` stays ignored. (`.claude/settings.json` must stay `{}` — a project-level `statusLine` overrides the user one.) `CLAUDEbackup.md` was deleted 07/29 after verifying 508 of its 509 unique lines existed elsewhere; the one exclusive line was a stale header.
